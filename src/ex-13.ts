...
const PokemonZod = z.object({
	name: z.string(),
	height: z.number(),
	weight: z.number(),
});
type PokemonType = z.infer<typeof PokemonZod>;

async function getPokemonBMI(name: string): Promise<number> {
	...
	const pokemonBMI = Math.round((weightInKg / (heightInM * heightInM)) * 100 / 100)
    
	return pokemonBMI;
}

const charmanderBMI = getPokemonBMI("charmander")
    .then((result) => console.log("Charmander's BMI is " + result)
    )
